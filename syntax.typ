...
#let rules = kleene.grammar(
  ...
  ident: {
    pat(maybe("."), <ident-startchar>, maybe(iter(<ident-anychar>)))
    rw(cs => (lab: cs.flatten().join()))
    yy(`LD_foo`, `main`, `loop0`, `.LD_xx`)
    nn(`0var`)
  },
  size: {
  ...
  realign: {
    pat(drop(".balign"), <xspaces>, <decvalue>)
    rw(n => (align: n.int))
  },
  value: {
  ...
  data-value: {
    pat(fork(<abstract-value>, <concrete-value>, <realign>))
    yy(
      `.byte 0x42`,
      `.skip 8`,
      `.balign 4`,
  ...
  instr-mov: {
    pat(fork("movb", "movh", "mov"))
    rw(instr => (instr: "mov", size: (mov: 4, movh: 2, movb: 1).at(instr)))
  },
  instr-any: {
    pat(fork("mvn", "add", "sub", "lsl", "lsr", "eor", "orr", "and", "b"))
    rw(instr => (instr: instr))
  },
  instr-code: {
    pat(fork(<instr-ldr>, <instr-mov>, <instr-any>))
    yy(
      `ldr`,
      `add`,
      `lsl`,
  ...
  print-directive: {
    ...
    rw(((w,rs),) => (meta: "print", width: w.at(0, default: auto), regs: rs))
    yy(
      `print/8 : r0`,
      `print : r1, r2`,
    )
  ...
  text-section: {
    ...
    rw(body => (text: body.flatten().filter(x => x != none)))
    yy(
      ```
        .text
...
#let arm(code) = {
  kleene.parse(rules, <arm>, code.text)
}
